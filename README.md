...
| `auto` | Auto-selected via cross-validation from several LOESS spans, several Huber `k` values, and a flat/no-op baseline. Each batch selects its own winner independently, from its own evidence only — different batches are expected to end up with different methods, nothing is pooled or shared across batches. QC-based selection: leave-one-out CV on that batch's own QC. Samples-based selection: a held-out prediction score against that batch's own ltQC (fit on Sample, then scored by how well the fitted curve predicts the actual ltQC values — a genuine train/test split, not D-ratio, since D-ratio can be gamed by a candidate that over-smooths Sample data and shrinks its own denominator; D-ratio is still computed and shown but no longer decides the winner). Works down to 2 ltQC per batch — held-out scoring has no leave-one-out minimum-size requirement on the ltQC side, unlike the QC tier's LOO-CV. | `AUTO_LOESS_SPANS`, `AUTO_HUBER_KS`, `AUTO_SAMPLE_LOESS_SPANS`, `AUTO_SAMPLE_HUBER_KS`, `AUTO_MIN_QC_PER_BATCH`, `AUTO_MIN_LTQC_VALIDATE`, `AUTO_MIN_CV_OBS` |

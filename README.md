...
| `loess_samples_limma` | Per-batch LOESS drift correction (QC-free — fit on biological samples instead of QC) followed by `limma::removeBatchEffect()` for between-batch correction. Same QC-free rationale as `loess_samples_combat` (robust fit, wider span, higher minimum-observation threshold, relies on randomised injection order), but with limma instead of ComBat for between-batch correction. | `LOESS_SAMPLE_SPAN`, `LOESS_SAMPLE_MIN_OBS` |

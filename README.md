...
| `WAVEICA_V1_T` | No | `0.05` | Comma-separated threshold(s) (0-1) for considering a component associated with batch in `batch=waveica_v1` — tested against real batch labels directly (a p-value cutoff), unlike `WAVEICA_CUTOFF`'s GAM-R²-against-injection-order proxy. Lower = more aggressive (more components removed); higher = more conservative. Single value fixed, multiple values searched |

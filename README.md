...
| `SPLINE_QC_SPAR` | No | `0.6` | Cubic smoothing spline (`stats::smooth.spline`) `spar` for `drift=spline`'s QC-based fit. Higher = smoother, more conservative. Fixed, not auto-searched — see `SPLINE_QC_CV_SPARS` for per-feature CV selection, or `AUTO_SPLINE_SPARS`/`drift=auto` for a per-batch CV-chosen `spar` instead |
| `SPLINE_SAMPLE_SPAR` | No | `0.8` | Smoothing spline `spar` for `drift=spline`'s samples-based fit. Higher than `SPLINE_QC_SPAR` by default — unlike LOESS/Huber, plain `smooth.spline` has no robust/outlier-resistant mode, so extra smoothing is this candidate's main defense against a single biological outlier |
...
| `AUTO_SPLINE_SPARS` | No | `0.4,0.6,0.8` | Comma-separated smoothing spline `spar` values `drift=auto` evaluates as candidates for the QC-based selection. Fixed grid points, like the other `AUTO_*` lists — not `smooth.spline`'s own internal CV, which would be a redundant nested CV on top of the outer per-batch selection |
...
| `AUTO_SAMPLE_SPLINE_SPARS` | No | `0.5,0.7,0.9` | Comma-separated smoothing spline `spar` values for `drift=auto`'s samples-based candidate pool. Higher range than `AUTO_SPLINE_SPARS`, since `smooth.spline` has no robust mode of its own |
...
| `AUTO_MIN_LTQC_VALIDATE` | No | `2` | Min ltQC samples a batch needs to select (and receive) its own `drift=auto` samples-based candidate at all. Under `basis=samples`, `hybrid`, and `qc` alike, a batch below this threshold has no held-out evidence to select from and is left uncorrected — each batch's selection depends only on its own ltQC/QC count, never on other batches |
...
| `SPLINE_QC_CV_SPARS` | No | (disabled) | Comma-separated smoothing spline `spar` candidates for `drift=spline`'s QC-based step. Same idea as `LOESS_QC_CV_SPANS`/`HUBER_QC_CV_KS` — per-feature leave-one-out CV instead of one shared `SPLINE_QC_SPAR` |
...
| `spline` | Cubic smoothing spline (`stats::smooth.spline`), fixed `spar` — the same fitting idea `notame_spline`/pmp's QCRSC use, but as a fixed-grid candidate rather than an atomic self-tuning method. More flexible than Huber's rigid line, smoother/more globally continuous than LOESS's local fit; unlike LOESS or Huber, plain `smooth.spline` has no robust/outlier-resistant mode. | `SPLINE_QC_SPAR`, `SPLINE_SAMPLE_SPAR`, `SPLINE_QC_CV_SPARS` |
| `auto` | Auto-selected via cross-validation from several LOESS spans, several Huber `k` values, several smoothing spline `spar` values, and a flat/no-op baseline, at two grains. **Which family** (loess/huber/spline) is decided once per tier (QC-tier and samples-tier separately, under `basis=hybrid`), pooling every eligible batch's own best-achievable-per-family score via **rank**, not raw score magnitude — each batch casts one vote for its preferred family regardless of its own noise level or feature count, so neither a high-variance batch nor an unusually feature-rich one can dominate the pooled comparison. **Which parameter** within that family — and whether to apply anything at all — is still decided independently per batch, from that batch's own evidence only (a different span/k/spar per batch is expected); `flat` stays available to every batch regardless of which family won. QC-based selection: leave-one-out CV on that batch's own QC. Samples-based selection: a held-out prediction score against that batch's own ltQC (fit on Sample, then scored by how well the fitted curve predicts the actual ltQC values — a genuine train/test split, not D-ratio, since D-ratio can be gamed by a candidate that over-smooths Sample data and shrinks its own denominator; D-ratio is still computed and shown but no longer decides the winner). Works down to 2 ltQC per batch — held-out scoring has no leave-one-out minimum-size requirement on the ltQC side, unlike the QC tier's LOO-CV. | `AUTO_LOESS_SPANS`, `AUTO_HUBER_KS`, `AUTO_SPLINE_SPARS`, `AUTO_SAMPLE_LOESS_SPANS`, `AUTO_SAMPLE_HUBER_KS`, `AUTO_SAMPLE_SPLINE_SPARS`, `AUTO_MIN_QC_PER_BATCH`, `AUTO_MIN_LTQC_VALIDATE`, `AUTO_MIN_CV_OBS` |
